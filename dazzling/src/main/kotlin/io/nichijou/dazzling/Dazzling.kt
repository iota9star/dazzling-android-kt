...
class Dazzling : BottomSheetDialogFragment() {
  ...
  var bgCornerRadii: FloatArray? = null
    set(value) {
      if (value != null && value.size != 8) throw IllegalArgumentException("bgCornerRadii size must be 8.")
      field = value
    }

  var paletteMargin: IntArray? = null
    set(value) {
      if (value != null && value.size != 4) throw IllegalArgumentException("paletteMargin size must be 4.")
      field = value
    }
  ...
  private fun initPaletteWidget() {
    ...
    mPaletteWrapper?.apply {
      this.background = (background as GradientDrawable).apply {
        ...
        this.cornerRadii = bgCornerRadii
      }
    }
    ...
    view.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
      override fun onGlobalLayout() {
        dialog.findViewById<FrameLayout>(com.google.android.material.R.id.design_bottom_sheet).let {
          ...
          if (paletteMargin != null) {
            (it.getChildAt(0).layoutParams as ViewGroup.MarginLayoutParams).setMargins(paletteMargin!![0], paletteMargin!![1], paletteMargin!![2], paletteMargin!![3])
          }
  ...
  private fun updateColor(color: Int) {
    ...
    if (mIsInit) {
      mIsInit = false
      mColorAdapter.newColors(presetColors ?: randomColors(randomSize).distinct(), preselectedColor)
    } else {
      mColorAdapter.newColors(color.stepColor(stepFactor).distinct(), color)
    }
  }
  ...
  private fun setDefault(isPreset: Boolean) {
    ...
    mColorAdapter.newColors(if (isPreset) presetColors!! else randomColors(randomSize).distinct(), mCurrentColor)
  }
  ...
  class Builder {
    ...
    fun setBgCornerRadii(cornerRadii: FloatArray?): Builder {
      dazzling.bgCornerRadii = cornerRadii
      return this
    }

    fun setPaletteMargin(paletteMargin: IntArray?): Builder {
      dazzling.paletteMargin = paletteMargin
      return this
    }
